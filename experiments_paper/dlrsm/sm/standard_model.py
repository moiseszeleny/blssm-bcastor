...
class SM:
    def __init__(
        ...
        #        )
        #    )[self.sm.complete_stack_ids]
        #self.cs_gg_h = np.array(
        #    self.sm.feature_vector(
        #        ['MG5','ggh','cross_section_pb']
        #        )
        #    )[self.sm.complete_stack_ids]
        ...
        #self._pred_br_h_aa = CubicSpline(self.mh, self.br_h_aa)
        #self._pred_cs_gg_h = CubicSpline(self.mh, self.cs_gg_h)
...
if __name__ == "__main__":
    # Example usage
    sm = SM('sm_mg5_full_processes_lep_200.json.gz')
    print(sm.sm.complete_stack_ids)
    print(sm.sm.feature_vector(
                ['SLHA','DECAY 25','entries','2,22,22', 'value']
                ))  # Print the loaded data for verification
    # Example Higgs mass for predictions
    mh = 125.0  # Example Higgs mass
    #print(sm.pred_br_h_aa(mh))
    #print(sm.pred_cs_gg_h(mh))
    print(sm.pred_cs_gg_h_aa(mh))
    print(sm.pred_cs_gg_h_tautau(mh))
    print(sm.pred_cs_ee_hz_zbb(mh))